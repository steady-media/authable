defmodule Authable.Model.TokenTest do
  ...
  test "changeset with valid attributes", %{user: user} do
    changeset = @token_store.changeset(%@token_store{}, %{user_id: user.id, details: %{grant_type: "unknown"}})
    assert changeset.valid?
    assert changeset.changes.value
  end

  test "changeset with invalid attributes" do
    changeset = @token_store.changeset(%@token_store{}, %{user_id: 123456})
    refute changeset.valid?
  end

  test "authorization_code_changeset", %{user: user} do
    changeset = @token_store.authorization_code_changeset(%@token_store{}, %{user_id: user.id, details: %{grant_type: "authorization_code"}})
    assert changeset.valid?
  ...
  test "refresh_token_changeset", %{user: user} do
    changeset = @token_store.refresh_token_changeset(%@token_store{}, %{user_id: user.id, details: %{grant_type: "authorization_code"}})
    assert changeset.valid?
  ...
  test "access_token_changeset", %{user: user} do
    changeset = @token_store.access_token_changeset(%@token_store{}, %{user_id: user.id, details: %{grant_type: "authorization_code"}})
    assert changeset.valid?
  ...
  test "session_token_changeset", %{user: user} do
    changeset = @token_store.session_token_changeset(%@token_store{}, %{user_id: user.id, details: %{grant_type: "openid"}})
    assert changeset.valid?
